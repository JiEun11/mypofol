...
select id, title, company, description, date_format(from_date, "%Y-%m") as fromDate, if(to_date is null, "재직", date_format(to_date, "%Y-%m")) as toDate, image_logo as imageLogo from experience where account_id=1;
...
select id, title, date_format(from_date, "%Y-%m") as fromDate, if(to_date is null, "재학중", date_format(to_date, "%Y-%m")) as toDate, degrees, image_logo as imageLogo from education where account_id=1 order by from_date desc;
...
select id, type, title, date_format(awarded, "%Y-%m") as awarded, ifnull(description, "") as description from training where account_id=1 order by type, awarded desc;
...
insert into project values(null, 'ERP 구매, eProcument 시스템', null, '포스코 계열사 및 공급사가 사용하는 ERP의 구매 시스템 고도화 및 운영', '/upload-images/4e1e019ba707a421922a4125bckaq130.png', '2023-01-00', null, now(), now(), 1);
insert into project values(null, 'AI 원료 예측 시스템', null, '적기적소의 원료 구매를 위한 원료 예측 시스템 개발 (포스코 홀딩스 AI 연구소 및 포스코 원료팀)', '/upload-images/4b1e019ya719a211122a9105bckpq232.png', '2022-06-00', null, now(), now(), 1);
insert into project values(null, '내부 회계 관리 시스템', null, '포스코 ERP 내부통제 프로세스가 반영된 내부 회계 관리 시스템 구축(신입 사원 입사 프로젝트)', '/upload-images/4e1e019ba707a421922a4125bckaq130.png', '2022-03-00', '2022-06-00', now(), now(), 1);
